  ...
  async function init(){
    ...
    try{
      ...
      list.innerHTML=manifest.issues.map((x,i)=>{
        ...
        const kind=x.kind==='legacy' ? '<span class="badge type">Legacy</span>' : '';
        return `<article class="article-card active history-item"><div><p class="eyebrow">${i===0?'LATEST ISSUE':'ARCHIVE'}</p><h2><a href="${esc(href)}">${esc(x.label)}</a></h2><div class="meta">${latest}${kind}</div></div><a class="history-open" href="${esc(href)}">開啟 →</a></article>`;
      }).join('');
